...
target = $appname.gsub(/[^a-z0-9]/i,'-') + '.md'
target = File.join('dist',target)
File.open(target,'w') do |f|
  f.puts menus
  $stdout.puts "Markdown written to #{target}"
end
